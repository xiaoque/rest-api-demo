...
const handle500Error = (json) => {
  ...
  const changeBootSize = (bootId, cb) =>{
     fetch(`boots/${bootId}/size/update`, {
      method: "PUT",
      headers: {
    ...
    boots.forEach((boot) => {
      ...
      bootsRow.innerHTML = `
        ...
        <td>
          ...
          <span class="update-size-icon" onclick="changeBootSize(${boot.id}, renderBootsListCallback(document.getElementById('bootsTableBody')));">❌</span>
        </td>
      `;
